...
async function handleDebugStates(adapter, source) {
    ...
    const states = {
        'sbms.timeStr': {
            name: 'Reported Timestamp of last Scrapping',
            unit: '',
            role: 'value',
            type: 'string',
        },
        'sbms.currentMA.battery': {
        ...
        'sbms.currentMA.extLoad': {
            name: 'External load current',
            unit: 'mA',
            role: 'value.current',
        },
        'sbms.soc': { name: 'State of charge', unit: '%', role: 'value.battery' },
        'sbms.tempInt': {
            name: 'SBMS Internal temperature',
            unit: '°C',
            role: 'value.temperature',
        },
        'sbms.tempExt': {
            name: 'External temperature (if connected)',
            unit: '°C',
            role: 'value.temperature',
        },
    ...
    for (let i = 1; i <= 8; i++) {
        states[`s2.cellsBalancing.${i}`] = {
            name: `Cell Balancing ${i}`,
            type: 'boolean',
            role: 'indicator',
        };
    }
    ...
    states['xsbms.cvmin'] = {
        name: 'Undervoltage Lock',
        unit: 'mV',
        role: 'value.voltage',
        type: 'number',
    };
    states['xsbms.cvmax'] = {
        name: 'Overvoltage Lock',
        unit: 'mV',
        role: 'value.voltage',
        type: 'number',
    };
    ...
    states['crcErrorCount'] = {
        name: 'CRC Error Count',
        unit: '',
        role: 'value',
        type: 'number',
    };
    states['crcSuccessCount'] = {
        name: 'CRC Success Count',
        unit: '',
        role: 'value',
        type: 'number',
    };
...
async function handleMqttDebugStates(adapter) {
    ...
    const states = {
        timeStr: {
            name: 'Reported Timestamp of last MQTT message',
            unit: '',
            role: 'value',
            type: 'string',
        },
