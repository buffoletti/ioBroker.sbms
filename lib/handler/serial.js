...
async function init(adapter, debug = false) {
    ...
    try {
        ...
        processSerialLine = (line) => {
            ...
            if (!modeLocked && ok) {
                adapter.log.info("Detected USART frame format (CRC valid).");
                modeLocked = true;
        ...
        timeout = setTimeout(async () => {
            if (!modeLocked) {
                adapter.log.warn(`No valid USART frame detected within 3s, falling back to WiFi mode.`);
...
function processSerialLineWifiMode(adapter, line, debug) {
    ...
    try {
        ...
        if (line.startsWith("var sbms=")) {
            ...
            matched = true;
        } else if (adapter.config.useUSART) {
...
function handleCompleteFrame(adapter, frame, debug) {
    ...
    try {
        ...
        if (sbms.timeStr == lastProcessedTimeStr) {
            if (debug) adapter.log.debug(`Serial frame skipped (${sbms.timeStr} is no new timeStr)`);
            return;
        }
        lastProcessedTimeStr = sbms.timeStr;

        if (debug) adapter.log.info(`Decoded complete serial frame, CRC good, Timestamp: ${sbms.timeStr}`);
