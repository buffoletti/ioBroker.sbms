...
async function init(adapter, debug = false) {
    ...
    try {
        ...
        timeout = setTimeout(() => {
            adapter.log.error(
                `No valid serial data received within 3s. Check baudrate (${baudRate}) or device ${device}`,
            );
            port.close();
        }, 3000);
...
function handleCompleteFrame(adapter, frame, debug) {
    ...
    if (now - lastProcessed < interval) {
        if (debug) adapter.log.debug(`Serial frame skipped (interval ${interval}ms not reached)`);
        return;
    }
    lastProcessed = now;
