...
let modeLocked = false; // USART or WIFI
...
function handleCompleteFrame(adapter, frame) {
    ...
    if (!checkIntegrity(frame.sbms)) {
        ...
        if (adapter.config.fullMessage && modeLocked) {
            adapter.incrementState('serial.crcErrorCount');
        }
        return;
    }

    if (adapter.config.fullMessage && modeLocked) {
        adapter.incrementState('serial.crcSuccessCount');
    }
