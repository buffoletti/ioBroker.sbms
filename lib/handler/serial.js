...
function handleCompleteFrame(adapter, frame, debug) {
    ...
    if (now - lastProcessed < interval) {
        if (debug) adapter.log.info(`Serial frame skipped (interval ${interval * 0.001}s not reached)`);
        return;
    }
    lastProcessed = now;
    ...
    // reset watchdog: if no CRC success in 10s → set connection false
...
function processDecoded(adapter, sbms, s1, s2, eW, xsbms) {
    ...
    if (anyBalancing) {
        for (let i = 1; i <= sbms.cellsMV.length; i++) {
            ...
            adapter.setState(`balancing.${i}`, v, true);
        ...
        adapter.setState("balancing.max.ID", maxID !== null ? maxID : 0, true);
        adapter.setState("balancing.min.ID", minID !== null ? minID : 0, true);
    }
    ...
    if (!anyBalancing) {
        ...
        adapter.setState("cells.min.ID", s2.cellsMin, true);
        adapter.setState("cells.max", sbms.cellsMV[s2.cellsMax - 1], true);
        adapter.setState("cells.max.ID", s2.cellsMax, true);
