...
function handleCompleteFrame(adapter, frame, debug) {
    ...
    clearTimeout(timeoutConnectionWatchdog);
    timeoutConnectionWatchdog = setTimeout(
        () => {
            adapter.setState("info.connection", false, true);
            adapter.log.warn(`No valid serial fram Wifi Mode for ${adapter.config.serialUpdateInterval * 5} s`);
