...
async function createNormalStates(adapter) {
    ...
    states['current'] = { type: 'channel', common: { name: 'Current fLow' } };
    states['power'] = { type: 'channel', common: { name: 'Power flow' } };
    ...
    states['cells'] = { type: 'channel', common: { name: 'Cell Voltages' } };
    for (let i = 1; i <= 8; i++) {
    ...
    states['flags.errors.activeErrors'] = {
        name: 'List of Active Error Flags',
        type: 'string',
        role: 'json',
    };
...
async function handleExtendedStates(adapter, remove = false) {
    ...
    // Counters
    states['counter'] = { type: 'channel', common: { name: 'Energy Counter' } };
    ...
    // Balancing states
    states['balancing'] = { type: 'channel', common: { name: 'Balancing Details' } };
    for (let i = 1; i <= 8; i++) {
    ...
    if (!(adapter.config.useHtml || adapter.config.useSerial) || remove) {
        for (const id of Object.keys(states)) {
            ...
            adapter.delObject(fullId, err => {
                if (err) {
                    adapter.log.warn(`Could not delete ${fullId}: ${err}`);
                }
            });
        }
        return; // exit early
    ...
    if (adapter.config.useHtml && adapter.config.useMQTT && !adapter.config.useSerial) {
        ...
        for (let i = 1; i <= 8; i++) {
            ...
            adapter.delObject(`balancing.${i}.active`, err => {
                if (err) {
                    adapter.log.warn(`Could not delete ${fullId}.active: ${err}`);
                }
            });
            adapter.delObject(`balancing.${i}.voltage`, err => {
                if (err) {
                    adapter.log.warn(`Could not delete ${fullId}.voltage: ${err}`);
                }
            });
        }

        // delete overall states
        const overall = ['anyActive', 'max', 'min', 'minID', 'maxID', 'activeCount'];
        overall.forEach(id => {
            adapter.delObject(`balancing.${id}`, err => {
                if (err) {
                    adapter.log.warn(`Could not delete balancing.${id}: ${err}`);
                }
            });
        });
    }
