...
async function handleExtendedStates(adapter) {
    ...
    // Balancing states
    for (let i = 1; i <= 8; i++) {
        states[`balancing.${i}`] = { name: `Cell ${i} Balancing Voltage`, unit: "mV", role: "value.voltage" };
    ...
    states[`balancing.min.ID`] = { name: "Cell ID min", unit: "", role: "value", type: "number" };
    states[`balancing.max.ID`] = { name: "Cell ID max", unit: "", role: "value", type: "number" };
    ...
    if (!(adapter.config.useHtml || adapter.config.useSerial)) {
...
module.exports = {
    createNormalStates,
    handleHtmlAdditionalStates: handleExtendedStates,
};
